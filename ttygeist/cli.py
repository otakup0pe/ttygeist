...
def find_socket_path(config_path: str | None = None) -> str:
    ...
    if "{pid}" in socket_pattern:
        ...
        if not matches:
            print(f"Error: No socket found matching pattern: {pattern}", file=sys.stderr)
            print("\nIs the MCP server running? Start it with:", file=sys.stderr)
            print("  uv run ttygeist", file=sys.stderr)
            sys.exit(1)
        elif len(matches) > 1:
            print(f"Error: Multiple sockets found: {matches}", file=sys.stderr)
            print("Please specify which one or stop extra servers.", file=sys.stderr)
            sys.exit(1)
        return matches[0]
...
def cmd_terminal(args):
    ...
    if args.verbose:
        ...
        logging.basicConfig(
            filename=str(log_file), level=logging.DEBUG, format="%(asctime)s - %(name)s - %(levelname)s - %(message)s"
        )
    ...
    try:
        ...
        if args.verbose:
            print(f"Terminal capabilities: color={color_support}, unicode={unicode_support}", file=sys.stderr)
...
def main():
    ...
    show_p = subparsers.add_parser("show", help="Show last N lines from buffer")
    show_p.add_argument("-n", "--lines", type=int, default=32)
    show_p.set_defaults(func=cmd_show)

    tail_p = subparsers.add_parser("tail", help="Tail buffer in real-time")
    tail_p.add_argument("-n", "--lines", type=int, default=0)
    tail_p.set_defaults(func=cmd_tail)

    status_p = subparsers.add_parser("status", help="Show connection and buffer status")
    status_p.set_defaults(func=cmd_status)

    term_p = subparsers.add_parser("terminal", help="Interactive terminal mode")
    term_p.add_argument("-n", "--lines", type=int, default=10)
    term_p.add_argument("-v", "--verbose", action="store_true")
